...
impl Walker {
    /// Visit every child of `element` in document order, recursing into
    /// non-target containers to find nested block elements.
    ...
    fn visit(&mut self, element: ElementRef) {
        match element.value().name() {
            ...
            // inside. nav/footer are intentionally NOT skipped here (see
            // the module doc comment's boilerplate caveat).
    ...
    fn visit_text_block(&mut self, element: ElementRef) {
        ...
        self.block_index += 1;
        let locator = match &self.current_heading_id {
            Some(id) => format!("#{id}"),
